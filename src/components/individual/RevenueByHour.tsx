...
import { Clock, TrendingUp, TrendingDown, PoundSterling, Users, Receipt, ChevronLeft, ChevronRight, AlertTriangle } from 'lucide-react';
          ...
          <>
            ...
            <div className="bg-purple-50 dark:bg-purple-900/30 rounded-lg p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-sm text-purple-600 dark:text-purple-400 font-medium">Daily Target</p>
                  <p className="text-2xl font-bold text-purple-900 dark:text-purple-100">{formatCurrency(selectedWeekData.weeklyTarget / 7)}</p>
                  <p className="text-xs text-purple-700 dark:text-purple-300">Average projection</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-amber-50 dark:bg-amber-900/30 rounded-lg p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-sm text-amber-600 dark:text-amber-400 font-medium">Forecast Model</p>
                  <p className="text-lg font-bold text-amber-900 dark:text-amber-100">Historical</p>
                  <p className="text-xs text-amber-700 dark:text-amber-300">+ seasonal trends</p>
                </div>
              </div>
            </div>
          </>
          ...
          <>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/30 rounded-lg p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-sm text-blue-600 dark:text-blue-400 font-medium">Daily Average</p>
                  <p className="text-2xl font-bold text-blue-900 dark:text-blue-100">{formatCurrency(selectedWeekData.weeklyTotal / 7)}</p>
                  <p className="text-xs text-blue-700 dark:text-blue-300">Target: {formatCurrency(selectedWeekData.weeklyTarget / 7)}</p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-900 dark:text-green-100">
                    {selectedWeekData.weekData.reduce((best, day) => 
                      day.dailyTotal > best.dailyTotal ? day : best
                    ).day}
                  </p>
                  <p className="text-xs text-green-700 dark:text-green-300">
                    {formatCurrency(Math.max(...selectedWeekData.weekData.map(d => d.dailyTotal)))}
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mb-6">
        ...
        <div className="space-y-2">
          ...
          {selectedDayData.hours.map((hour, index) => (
            <div key={index} className="flex items-center space-x-4 p-2 bg-white dark:bg-gray-600 rounded border border-gray-200 dark:border-gray-500">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center space-x-2 text-xs text-gray-600 dark:text-gray-400">
                    {selectedWeekData.weekType === 'forecast' ? (
                      <>
                        <span className="text-blue-600">{hour.transactions} txns (est.)</span>
                        <span className="text-blue-600">£{hour.avgTransaction} (est.)</span>
                      </>
                    ) : (
                      <>
                        <span>{hour.transactions} txns</span>
